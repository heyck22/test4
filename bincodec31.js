let ByteBuffer = (this, function(Long) {
    "use strict";
...
BinCodec.prototype.decodePreEnterWorldResponse = function(buffer) {
    console.log(JSON.stringify(Array.from(new Uint8Array(buffer.buffer))))
    for (let i = 0; i < HEAPU8.length; i++) {
        HEAPU8[i] = HEAPU83[i];
    }
  asm_l._MakeBlendField(24, 132)
    for (var firstSync = asm_l._MakeBlendField(228, 132), i = 0; buffer.remaining(); )
        HEAPU8[firstSync + i] = buffer.readUint8(), i++;
    asm_l._MakeBlendField(172, 36)
    for (var secondSync = 5248336, extraBuffers = new ArrayBuffer(64), exposedBuffers = new Uint8Array(extraBuffers), i = 0; i < 64; i++) {
        exposedBuffers[i] = HEAPU8[secondSync + i];
    }
